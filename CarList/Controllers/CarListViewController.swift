...
final class CarListViewController: UIViewController, deleteProtocol
{
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if segue.identifier == "showCar" {
			guard let newCarVC = segue.destination as? NewCarTableViewController else { return }
...
extension CarListViewController
{
	func loadDefaultCars() {
		if UserDefaults.standard.value(forKey: "firstRun") as? Bool != nil {
			return
		}
		else {
			UserDefaults.standard.set(false, forKey: "firstRun")
			_ = storageManager.saveCar(model: "Mustang",
										   manufacturer: "Ford",
										   bodyType: "Coupe",
										   yearOfManufacture: Date(timeIntervalSinceNow: 0),
										   imageData: UIImage(named: "Mustang")?.pngData())
			_ = storageManager.saveCar(model: "Camaro",
										   manufacturer: "Chevrolet",
										   bodyType: "Coupe",
										   yearOfManufacture: Date(timeIntervalSince1970: 700000000),
										   imageData: UIImage(named: "Camaro")?.pngData())
			_ = storageManager.saveCar(model: "E200",
										   manufacturer: "Mercedes-Benz",
										   bodyType: "Sedan",
										   yearOfManufacture: Date(timeIntervalSince1970: 900000000),
										   imageData: UIImage(named: "E200")?.pngData())
		}
	}
}
